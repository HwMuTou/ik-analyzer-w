...
public class IKQueryExpressionParser {
        ...
        if ('&' == op.type) {
            ...
            if (q2 != null) {
                if (q2 instanceof BooleanQuery) {
                    ...
                    if (!clauses.isEmpty() && clauses.get(0).getOccur() == Occur.MUST) {
                        for (BooleanClause c : clauses) {
                            builder.add(c);
                        }
                    } else {
            ...
            buildQuery(q1, builder, Occur.SHOULD);
            buildQuery(q2, builder, Occur.SHOULD);
        } else if ('-' == op.type) {
            if (q1 == null || q2 == null) {
    ...
    private void buildQuery(Query query, BooleanQuery.Builder builder, Occur occur) {
        if (query != null) {
