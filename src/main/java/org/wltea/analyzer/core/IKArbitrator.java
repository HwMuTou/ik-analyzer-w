...
class IKArbitrator {
    ...
    void process(AnalyzeContext context, boolean useSmart) {
        ...
        while (orgLexeme != null) {
            if (!crossPath.addCrossLexeme(orgLexeme)) {
                ...
                lexemePath(crossPath, useSmart, context);
    ...
    private void lexemePath(LexemePath crossPath, boolean useSmart, AnalyzeContext context) {
    ...
    private LexemePath judge(QuickSortSet.Cell lexemeCell) {
        ...
        //存在歧义词，处理
        QuickSortSet.Cell cell;
        while (!lexemeStack.isEmpty()) {
            cell = lexemeStack.pop();
            //回滚词元链
            this.backPath(cell.getLexeme(), option);
            //从歧义词位置开始，递归，生成可选方案
            this.forwardPath(cell, option);
            pathOptions.add(option.copy());
        }
